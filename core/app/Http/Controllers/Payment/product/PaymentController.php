...
class PaymentController extends Controller
{
    ...
    public function orderTotal($scharge) {
        ...
        $total = ($total + $shippig_charge + tax()) - $discount;
        $total = round($total, 2);
    ...
    public function orderValidation($request) {
        ...
        if ($request->serving_method == 'home_delivery' && $be->delivery_date_time_status == 1 && $be->delivery_date_time_required == 1) {
            $rules['delivery_date'] = 'required|sometimes';
            $rules['delivery_time'] = [
                'required',
                function ($attribute, $value, $fail) use ($request) {
                    ...
                    if (!empty($tf) && $tf->max_orders != 0) {
                        $orderCount = ProductOrder::where('completed', 'no')->where('order_status', '<>', 'cancelled')->where('delivery_time_start', $tf->start)->where('delivery_time_end', $tf->end)->count();
    ...
    public function saveOrder($request, $shipping, $total, $txnId=NULL, $chargeId=NULL) {
        if (session()->has('lang')) {
